...
import { teacherSchema, teacherCreateSchema, TeacherSchema } from "@/lib/formValidationSchemas";
...
const TeacherForm = ({
  ...
  const action = async (formData: FormData) => {
    startTransition(async () => {
      try {
        ...
        data.subjects = formData.getAll("subjects").map(id => parseInt(id as string));
        data.birthday = new Date(data.birthday);
        if (data.id) data.id = data.id;

        const result = type === "create" ? await createTeacher(data) : await updateTeacher(data);
  ...
  return (
    <form className="flex flex-col gap-8" action={action}>
      ...
      {state.error && (
        <span className="text-red-500">{(state as any).message || "Something went wrong!"}</span>
      )}
